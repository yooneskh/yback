...
export async function sendDirectSMS(options: DirectSMSOptions) {
  ...
  const url = `https://api.kavenegar.com/v1/${Config.notifications.kavenegar.apiKey}/sms/send.json?receptor=${options.receptors.join(',')}&sender=${options.sender}&message=${encodeURIComponent(options.message)}`;
